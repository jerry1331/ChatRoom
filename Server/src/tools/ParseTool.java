		...
		if (msgtype == 0x01) {// 注册信息
			...
			for (i = 0; i < listCount; i++) {
				...
				for (j = 0; j < bodyCount[i]; j++) {
					...
					System.out.println(
							bodyNum[i][j] + " " + bodyPic[i][j] + " " + nikeName[i][j] + " " + bodyState[i][j]);
				}
