...
public class ChatClient extends Thread {
	...
	private static int OwnIDNum;// 当登陆成功后，就该ChatClient的唯一ID号
	...
	public boolean Reg(String NikeName, String PassWord) {
		try {
			...
			if (mrr.getState() == 0) {
				 ...
				 * 注册成功
				 */
				// System.out.println("注册的ID为" + mrr.getDest());
...
//				System.out.println("To show Result");
	...
	public void printList(ListInfo list) {
		...
		System.out.print(" ID " + list.getIDNum());
		...
		for (i = 0; i < listCount; i++) {
			...
			for (j = 0; j < bodyCount[i]; j++) {
				System.out.print(" ID " + bodyNum[i][j]);
